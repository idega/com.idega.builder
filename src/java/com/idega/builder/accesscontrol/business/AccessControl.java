package com.idega.builder.accesscontrol.business;
...
import com.idega.builder.accesscontrol.data.*;
...
public class AccessControl{
    ...
    public static boolean isAdmin(ModuleInfo modinfo)throws SQLException{
      Member member = LoginBusiness.getMember(modinfo);

      if (member != null){
        List access = member.getPermissionGroups();
        if (access != null){
    ...
    public static boolean hasPermission(String permissionType, ModuleObject obj,ModuleInfo modinfo) throws SQLException{
      ...
      if (isAdmin(modinfo)){
        ...
        PermissionGroup[] groups = getPermissionGroups(member);
        String sGroupList = "";
...
//        permission.setPermissionStringValue();
    ...
    public void createPermissionGroup(String GroupName, String Description, String ExtraInfo, int[] memberIDs, int[] groupIDs)throws SQLException{
      ...
      if(memberIDs != null){
        for (int i = 0; i < memberIDs.length; i++) {
          addMemberToPermissionGroup(newGroup, memberIDs[i]);
        }
      }
      if (groupIDs != null){
    ...
    public void addMemberToPermissionGroup(PermissionGroup group, int memberIDtoAdd) throws SQLException{
      Member memberToAdd = new Member(memberIDtoAdd);
    ...
    public static PermissionGroup[] getPermissionGroups(Member member) throws SQLException{
      Group[] groups = PermissionGroup.getStaticPermissionGroupInstance().getAllGroupsContainingMember(member);
